...
void init_arrF(double a[], int i, int j);
...
void main()
{
  ...
  double A[128];
  init_arrF(A,0,dim-1);
  printf("before FFT: %1.0f-%1.0f-..\n",A[0],A[1]);
  FFT_transform(N,A);
  printf("after FFT: %1.0f-%1.0f-..\n",A[0],A[1]);
//  FFT_inverse(N,A);
//  printf("after FFT_inverse: %1.0f-%1.0f-..\n",A[0],A[1]);
}
...
if (ii+1<0 || ii+1>=dim) { error(); };
        double tmp_imag    = data[ii+1];
if (ii<0 || ii>=dim) { error(); };
if (jj<0 || jj>=dim) { error(); };
        data[ii]   = data[jj];
if (ii+1<0 || ii+1>=dim) { error(); };
if (jj+1<0 || jj+1>=dim) { error(); };
        data[ii+1] = data[jj+1];
if (jj<0 || jj>=dim) { error(); };
        data[jj]   = tmp_real;
if (jj+1<0 || jj+1>=dim) { error(); };
